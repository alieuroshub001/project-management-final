// app/api/employee/projects/[projectId]/tasks/[taskId]/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { getServerSession } from 'next-auth/next';
import connectToDatabase from '@/lib/db';
import { Task, ITaskDocument } from '@/models/employee/Project/Task';
...
import { Comment } from '@/models/employee/Project/Comment';
...
export async function GET(
  ...
  try {
    ...
    const hasAccess = await hasProjectAccess(params.projectId, session.user.id);
    if (!hasAccess) {
      return NextResponse.json<IProjectApiResponse>({
        success: false,
    ...
    const [assignedToDetails, subtasks, comments, timeEntries, dependentTasks] = await Promise.all([
      ...
      Task.find({ parentTaskId: params.taskId }),
      Comment.findByTask(params.taskId),
      TimeEntry.find({ taskId: params.taskId }),
...
export async function DELETE(
  ...
  try {
    ...
    const subtasks = await Task.find({ parentTaskId: params.taskId });
    ...
    await Task.updateMany(
      { dependencies: params.taskId },
      { $pull: { dependencies: params.taskId } }
    );
