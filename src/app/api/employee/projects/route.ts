...
import Project, { IProjectDocument } from '@/models/employee/Project/Project';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const managerName = projectManager ? 
      (await import('@/models/employee/User')).default.findById(projectManager)?.name || session.user.name :
      session.user.name;
    const managerEmail = projectManager ?
      (await import('@/models/employee/User')).default.findById(projectManager)?.email || session.user.email :
      session.user.email;
